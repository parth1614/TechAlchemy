...
import TechAlchemy from '../src/abis/TestToken.json';
import APYStaking from '../src/abis/TokenStaking.json';
...
const App = () => {
  ...
  const fetchDataFromBlockchain = async () => {
    if (window.ethereum) {
      ...
      const testTokenData = APYStaking.networks[networkId];
      if (testTokenData) {
        let web3 = window.web3;
        ...
        //  fetching balance of TechAlchemy and storing in state
        ...
        const tempBalance = APYStaking.networks[networkId];
        ...
        setAppStatus(false);
        window.alert(
          'TechAlchemy contract is not deployed on this network, please change to testnet'
        );
      }

      //loading TokenStaking contract data
      const tokenStakingData = APYStaking.networks[networkId];
      ...
      if (tokenStakingData) {
        ...
        let tempApy =
          ((await tokenStaking.methods.thirtyAPY().call()) / 1000) * 365;
        let tempcustomApy60 =
          ((await tokenStaking.methods.sixtyAPY().call()) / 1000) * 365;
        let tempcustomApy90 =
            ((await tokenStaking.methods.APY90().call()) / 1000) * 365;
